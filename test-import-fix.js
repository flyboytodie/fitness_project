...
|------|----------|----------|----------|------|------|--------|------|
| 2026-04-13 | 胸+三头 | 杠铃卧推 | 60 | 4 | 8/5/4/4 | 21 | 60kg偏重 |
| 2026-04-13 | 胸+三头 | 上斜哑铃卧推 | 22.5 | 4 | 8/4/8/8 | 28 | 第2组无力 |
| 2026-04-14 | 背+二头 | 引体向上 | 0 | 4 | 6/5/4/4 | 19 | 起点 |

## 🥗 饮食记录

| 日期 | 千卡 | 蛋白质(g) | 碳水(g) | 脂肪(g) | 备注 |
|------|------|-----------|---------|---------|------|
| 2026-04-13 | 1800 | 105 | 150 | 0 | 晚餐吃撑 |
| 2026-04-14 | 1480 | 142 | 134 | 0 | 晚餐吃撑 |
`;
...
class TestSettingsModule {
  parseMarkdownToData(content) {
    ...
    for (const line of lines) {
      if (line.startsWith('## 📅') || line.startsWith('## 训练记录')) {
        currentSection = 'workout';
        inTable = false;
        continue;
      }
      
      if (line.startsWith('## 🥗') || line.startsWith('## 饮食记录')) {
        currentSection = 'diet';
        inTable = false;
        continue;
      }
      
      if (line.startsWith('## 📊') || line.startsWith('## 身体数据')) {
        currentSection = 'body';
        inTable = false;
        continue;
      }
...
const result = settingsModule.parseMarkdownToData(testMarkdown);
...
result.workouts.forEach(w => {
  console.log(`日期: ${w.date}, 部位: ${w.parts.join('+')}, 动作数: ${w.exercises.length}`);
...
result.diet.forEach(d => {
  console.log(`日期: ${d.date}, 热量: ${d.calories}kcal, 蛋白质: ${d.protein}g`);
});